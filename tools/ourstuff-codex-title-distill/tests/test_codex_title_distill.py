...
MODULE_PATH = Path(__file__).resolve().parents[2] / "local-ai-brain" / "local_ai_brain" / "title_distill.py"
SPEC = importlib.util.spec_from_file_location("codex_title_distill", MODULE_PATH)
codex_title_distill = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
sys.modules[SPEC.name] = codex_title_distill
SPEC.loader.exec_module(codex_title_distill)
