...
- Delegated linear-thinking agents must not consume standard Codex/GPT-5.5 usage. Use Spark only through an explicit model-selecting command such as `codex exec --model gpt-5.3-codex-spark --cd <repo> "<prompt>"`; otherwise use GitHub Copilot when available or OpenCode/free helpers.
- Do not use `multi_agent_v1.spawn_agent` for Spark lanes unless it exposes explicit non-standard model selection and actual model provenance. If actual model is unknown, do not claim Spark.
...
- If Spark is unavailable, unsuitable, or cannot be selected, route delegated work through GitHub Copilot if available, then OpenCode/free helpers. If no non-standard delegated backend is available, Blue/Logos may continue direct orchestration or small direct implementation, but must not describe that as delegated agent work.
...
- OpenCode-backed runs must follow the role model map in `references/opencode-agent-backend.md`: MiMo-V2.5 for research, MiniMax M2.7 for building, and GPT-OSS free models for worker/review fallback.
...
- Never use paid OpenCode aliases for a free-only run. If the requested role model is unavailable, unverified, blocked, or not free-labeled, record a backend failure note and use a verified free fallback for that purpose when one is listed; otherwise stop delegation and keep only direct Blue/Logos orchestration instead of spending standard Codex usage.
