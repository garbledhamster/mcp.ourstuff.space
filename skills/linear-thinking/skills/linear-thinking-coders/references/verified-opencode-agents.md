...
| --- | --- | --- | --- | --- |
| Worker/search/summaries | DeepSeek V4 Flash | `opencode/deepseek-v4-flash-free` | `Say READY.` returned `READY.` | Cheap worker throughput, scouting, summaries, narrow repetitive subtasks, simple smoke prompts. |
...
```powershell
opencode run --model "deepseek/deepseek-v4-flash:free" "Say READY."
...
- Paid aliases such as `opencode/qwen3.6-plus`, `opencode/minimax-m2.7`, or `opencode/deepseek-v4-flash` are not allowed in a free-only run.

## Dispatch Rules

- Worker/search/summaries: use DeepSeek V4 Flash free when verified; otherwise use another verified free worker only if Blue/Logos records why it fits the same purpose.
- Long-context reader: use MiMo-V2.5 free when verified; otherwise keep broad synthesis local.
...
- Reviewer/validator: prefer Qwen3.6 Plus free only after it is verified. Until then, use the verified free reviewer fallback listed above or keep review local.
