...
Fallback rule: if OpenCode fails to start, cannot use the selected model, times out, edits the wrong scope, returns unusable output, or cannot provide both pair perspectives, Codex takes the lane back while preserving the pair roles. When explicit model selection is available, the returned coding lane uses GPT-5.3-Codex-Spark (`gpt-5.3-codex-spark`) with reasoning effort `high` by default or `xhigh` for complex, risky, cross-module, architecture-sensitive, or user-facing work. GPT-5.5/Blue/Logos writes the lane brief and resumes direct execution only when Spark is unavailable, unsuitable, or cannot be selected.
...
| --- | --- | --- | --- |
| Worker drone | DeepSeek V4 Flash | High-throughput scouting, simple implementation substeps, repetitive narrow edits, smoke prompts | Very high request limits, fast, cheap, good-enough reasoning. |
...
| Reviewer | Qwen3.6 Plus | Cross-review, critique, validation, acceptance-check review, risk analysis | Strong reasoning-to-cost ratio for critique and validation. |
...
- `LINEAR_THINKING_CODERS_OPENCODE_WORKER_MODEL`: DeepSeek V4 Flash provider/model ID.
...
- `LINEAR_THINKING_CODERS_OPENCODE_REVIEWER_MODEL`: Qwen3.6 Plus provider/model ID.

Do not use older generic env vars such as `LINEAR_THINKING_CODERS_OPENCODE_THINKING_MODEL` or `LINEAR_THINKING_CODERS_OPENCODE_CODING_MODEL` for new dispatch. If those are present, ignore them unless they resolve to one of the four required role models.
...
7. If the required model is unavailable, cannot be resolved, fails preflight, is provider-blocked, is not free-labeled during a free-only run, or cannot perform the lane reliably, record an OpenCode backend failure note and use the verified free fallback for that purpose when one is listed. If no verified fallback fits, return the lane to Codex Spark first, with GPT-5.5/Blue/Logos retaining orchestration and review.
...
- Worker-drone prompts use `LINEAR_THINKING_CODERS_OPENCODE_WORKER_MODEL`.
...
If the env var is empty, resolve the exact model ID from the refreshed OpenCode model list. If that fails, record failure and fall back to a Codex Spark coding lane first, with GPT-5.5/Blue/Logos retaining orchestration and review.
...
- `Command shape:`
- `Failure evidence:`
- `Fallback used:` Codex Spark coding lane, or GPT-5.5/Blue/Logos if Spark was unavailable
- `Future guidance:`
```

Then continue with Codex as the pair executor, using Spark first when selectable. Do not stop the whole workflow unless the failure reveals a missing user decision, missing credential, or unsafe repo state.
